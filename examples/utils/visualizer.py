...
class Visualizer(object):
    ...
    def vis_scalar(self, win_name, trace_name, x, y, opts=None):
        ...
        if win is not None:
            self.vis.line( X=x, Y=y, opts=default_opts, update='append',win=win, name=trace_name )
        else:
            self.cur_win[win_name] = self.vis.line( X=x, Y=y, opts=default_opts, name=trace_name)
